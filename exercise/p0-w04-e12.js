...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++) {
    ...
    for (let j = 0; j < shoppers.length; j++) {
        if (shoppers[j].product === listBarang[i][0] && sold + shoppers[j].amount <= listBarang[i][2]) {
    ...
    temp.shoppers = shopper
    temp.leftOver = listBarang[i][2] - sold
    temp.totalProfit = listBarang[i][1] * sold
    result.push(temp)
  }
  return result
}
